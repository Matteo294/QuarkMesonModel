    ...
    lam(lam)
{
    for(int nt=0; nt<Nt; nt++){
        for(int nx=0; nx<Nx; nx++){
            phi[nt][nx] = Eigen::Vector4cd {1.0, 2.0, 3.0, 4.0};
        }
    }
}

std::complex<double> O4Mesons::evaluateDrift(int const nt, int const nx, int const j){
    std::complex<double> K = - phi[PBC(nt+1, Nt)][nx][j] - phi[PBC(nt-1, Nt)][nx][j] - phi[nt][PBC(nx+1, Nx)][j] - phi[nt][PBC(nx-1, Nx)][j] + phi[nt][nx][j] * (m2 + 4.0) + lam/6.0 * pow(phi[nt][nx][j], 3);
    for(int i=0; i<4; i++){
        if (i != j){
...
double O4Mesons::norm(){
    ...
    for(int nt=0; nt<Nt; nt++){
        for(int nx=0; nx<Nx; nx++){
            //std::cout << sqrt((phi[nt][nx][0]*conj(phi[nt][nx][0]) + phi[nt][nx][1]*conj(phi[nt][nx][1]) + phi[nt][nx][2]*conj(phi[nt][nx][2]) + phi[nt][nx][3]*conj(phi[nt][nx][3])).real()) << std::endl;
