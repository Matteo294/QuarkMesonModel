...
__global__ void D_oo(Spinor<T> *inVec, Spinor<T> *outVec, MatrixType const useDagger, thrust::complex<T> *M){
    ...
    for (int i = grid.thread_rank() + vol/2; i < vol; i += grid.size()) {
        if (useDagger == MatrixType::Dagger){
            ...
            outVec[i].val[2] += (two + mass + g * conj(M[i])) * inVec[i].val[2];
            outVec[i].val[3] += (two + mass + g * conj(M[i])) * inVec[i].val[3];
        } else{
            outVec[i].val[0] += (two + mass + g * (M[i])) * inVec[i].val[0];
            outVec[i].val[1] += (two + mass + g * (M[i])) * inVec[i].val[1];
            outVec[i].val[2] += (two + mass + g * (M[i])) * inVec[i].val[2];
            outVec[i].val[3] += (two + mass + g * (M[i])) * inVec[i].val[3];
        }
    }
}
