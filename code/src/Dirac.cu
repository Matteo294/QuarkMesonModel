...
__global__ void D_oo(Spinor<T> *inVec, Spinor<T> *outVec, MatrixType const useDagger, thrust::complex<T> *M){
    ...
    for (int i = grid.thread_rank() + vol/2; i < vol; i += grid.size()) {
        ...
        if (useDagger == MatrixType::Dagger){
            outVec[i].val[0] +=  im_gpu * g * ( conj((M1[i] - sigma)) * inVec[i].val[1] +         conj(M3[i])     * inVec[i].val[3] ) + (two + mass + g*conj(sigma)) * inVec[i].val[0];
            outVec[i].val[1] += -im_gpu * g * ( conj((M1[i] - sigma)) * inVec[i].val[0] +         conj(M3[i])     * inVec[i].val[2] ) + (two + mass + g*conj(sigma)) * inVec[i].val[1];
            outVec[i].val[2] +=  im_gpu * g * (       conj(M2[i])     * inVec[i].val[1] + conj((M4[i] - sigma))   * inVec[i].val[3] ) + (two + mass + g*conj(sigma)) * inVec[i].val[2];
            outVec[i].val[3] += -im_gpu * g * (       conj(M2[i])     * inVec[i].val[0] + conj((M4[i] - sigma))   * inVec[i].val[2] ) + (two + mass + g*conj(sigma)) * inVec[i].val[3];
        } else{
            outVec[i].val[0] +=  im_gpu  * g * ( (M1[i] - sigma) * inVec[i].val[1] +         M2[i]     * inVec[i].val[3] ) + (two + mass + g*sigma) * inVec[i].val[0];
            outVec[i].val[1] += -im_gpu * g * ( (M1[i] - sigma) * inVec[i].val[0] +         M2[i]     * inVec[i].val[2] ) + (two + mass + g*sigma) * inVec[i].val[1];
            outVec[i].val[2] +=  im_gpu * g * (       M3[i]     * inVec[i].val[1] + (M4[i] - sigma)   * inVec[i].val[3] ) + (two + mass + g*sigma) * inVec[i].val[2];
            outVec[i].val[3] += -im_gpu * g * (       M3[i]     * inVec[i].val[0] + (M4[i] - sigma)   * inVec[i].val[2] ) + (two + mass + g*sigma) * inVec[i].val[3];
        }
    }
}
...
__global__ void D_ee_inv(Spinor<T> *inVec, Spinor<T> *outVec, MatrixType const useDagger, thrust::complex<T> *M){
    ...
    if (useDagger == MatrixType::Dagger){
        outVec[i].val[0] +=  im_gpu * ( conj((Minv[0] - sigma))    * inVec[i].val[1] +         conj(Minv[2])     * inVec[i].val[3] ) + sigma * inVec[i].val[0];
        outVec[i].val[1] += -im_gpu * ( conj((Minv[0] - sigma))    * inVec[i].val[0] +         conj(Minv[2])     * inVec[i].val[2] ) + sigma * inVec[i].val[1];
        outVec[i].val[2] +=  im_gpu * (          conj(Minv[1])     * inVec[i].val[1] + conj((Minv[3] - sigma))   * inVec[i].val[3] ) + sigma * inVec[i].val[2];
        outVec[i].val[3] += -im_gpu * (          conj(Minv[1])     * inVec[i].val[0] + conj((Minv[3] - sigma))   * inVec[i].val[2] ) + sigma * inVec[i].val[3];
    } else {
        outVec[i].val[0] +=  im_gpu * ( (Minv[0] - sigma) * inVec[i].val[1] +      Minv[1]       * inVec[i].val[3] ) + sigma * inVec[i].val[0];
        outVec[i].val[1] += -im_gpu * ( (Minv[0] - sigma) * inVec[i].val[0] +      Minv[1]       * inVec[i].val[2] ) + sigma * inVec[i].val[1];
        outVec[i].val[2] +=  im_gpu * (       Minv[2]     * inVec[i].val[1] + (Minv[3] - sigma)  * inVec[i].val[3] ) + sigma * inVec[i].val[2];
        outVec[i].val[3] += -im_gpu * (       Minv[2]     * inVec[i].val[0] + (Minv[3] - sigma)  * inVec[i].val[2] ) + sigma * inVec[i].val[3];
    }
    }
}
