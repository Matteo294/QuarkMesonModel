...
//		e = static_cast<myType>(drand48());
	...
	Spinor<double> in, out; // spinors useful to e.g. apply Dirac operator
	DiracOP<double> Dirac; // Dirac operator
	FermionicDrift fDrift(seed); // FermionicDrift
	double *condensate; // Chiral condensate
	CGsolver CG; // Conjugate Gradient solver
    double *trace; // trace D^-1 ---> same as condensate?
	int myvol = spinor_vol; // dynamic volume
	cp<double> *corr;
	
	// allocate spinors in managed memory
	cudaMallocManaged(&fermionic_contribution, sizeof(double) * vol); 
	cudaMallocManaged(&trace, sizeof(double));
    cudaMallocManaged(&corr, nTimeSlices * sizeof(double));
    ...
    // prepare kernel calls: compute number of block and threads for each kernel
	int nBlocks = 0;
	int nThreads = 0;
    ...
    nBlocks = 0;
	nThreads = 0;
	cudaOccupancyMaxPotentialBlockSize(&nBlocks, &nThreads, gpuTimeSlicesFermions);
	cudaDeviceSynchronize();
	auto dimGrid_slices = dim3(nBlocks, 1, 1);
	auto dimBlock_slices = dim3(nThreads, 1, 1);
    void *slicesArgs[] = {(void*) &in.data(), (void*) &corr, (void*) &spinor_vol};
    
	// set up print files
	std::ofstream datafile, tracefile;
	datafile.open("correlator.csv");
	datafile << "corr" << "\n";
	std::string fname;
	fname.append("avg_fields"); fname.append(".csv");
...
//		cudaMemPrefetchAsync(ivec.data(), N*nVectorComponents, cudaCpuDeviceId);
        ...
        slicesArgs[0] = (void*) &in.data();
        slicesArgs[1] = (void*) &corr;
        slicesArgs[2] = (void*) &spinor_vol;
        cudaLaunchCooperativeKernel((void*) &gpuTimeSlicesFermions, dimGrid_timeSlices, dimBlock_timeSlices, slicesArgs, 32 * sizeof(double), NULL);
		cudaDeviceSynchronize();
		/*cp<double> corr;
        ...
        // note that since <psibar psi> = Tr(D^-1) and the drift force is g Tr(D^-1), I can compute the condensate by diving the drift force by g (when g != 0)
        fDrift.getForce(drift.data(), Dirac, CG, dimGrid_drift, dimBlock_drift);        
        *condensate = 0.0;
        ...
        if (yukawa_coupling != 0.0) { 
            *condensate /= yukawa_coupling;
        } else {
            *condensate = 0.0;
        }
		
        tracefile << (double) (*condensate) / vol << "," << (double) (avg[0] / vol) << "," << (double) (std::sqrt(sum2) / vol) << "\n";
		// ------------------------------------------------------
